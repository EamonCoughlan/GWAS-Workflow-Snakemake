...
```{r, echo=FALSE}
lmiss <- read.table(snakemake@input[[4]], header = TRUE, stringsAsFactors = FALSE)
Total <- nrow(lmiss)
Removed <- nrow(lmiss[lmiss$F_MISS>snakemake@params[['geno2value']],])
...
for (i in 3:(as.numeric(snakemake@params[['PCnumber']])+1)) {
  ...
  legend('topleft', legend = unique(values$V3), fill = as.factor(unique(values$V3)))
}
```